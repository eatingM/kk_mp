...
class AddSinglePageTextTitleTest(MyTest):
    ...
    def test_023_edit_single_title(self):
        ...
        actual_value = SourceManagePage(self.driver).get_mp_single_shown_title_text()
    ...
    def test_029_edit_single_title(self):
        """用例编号029：mp登录后，进入素材管理的图文界面，添加单图文的标题(英文短特殊字符输入)"""
    ...
    def test_058_edit_single_title(self):
        """用例编号058：mp登录后，进入素材管理的图文界面，添加单图文的标题(中文短特殊字符输入)"""
        expected_value = self.add_single_page_text_title("058")
        logger.info("获得的测试用例期望值为： %s" % expected_value)
        actual_value = SourceManagePage(self.driver).get_mp_single_shown_title_text()
        logger.info(("获得的测试用例实际值为： %s" % actual_value))
        logger.info("测试判断:%s 是否等于:%s" % (actual_value, expected_value))
    ...
    def test_041_edit_single_title(self):
        ...
        logger.info("继续输入内容: %s" % input_value)
